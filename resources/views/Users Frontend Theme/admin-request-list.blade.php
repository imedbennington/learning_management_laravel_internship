                ...
                <tbody>
                    @foreach($demands as $demand)
                        <tr>
                            ...
                            <td>{{ $demand->phone }}</td>
                            <td>{{ $demand->date_of_birth }}</td>
                            <td>{{ $demand->created_at }}</td>
                            <td>{{ $demand->status }}</td>
                            <td>
                                @if($demand->status === 'pending')
                                    <form action="{{ route('admin.demands.approve', $demand->id) }}" method="POST"
                                        style="display:inline;">
                                        @csrf
                                        <button type="submit" class="btn btn-success btn-sm">Approve</button>
                                    </form>

                                    <form action="{{ route('admin.demands.cancel', $demand->id) }}" method="POST"
                                        style="display:inline;">
                                        @csrf
                                ...
                                @else
                                    ...
                                    <!-- Optional: display a message if the demand has already been processed -->
                                    <span>Already {{ $demand->status }}</span>
                                @endif
